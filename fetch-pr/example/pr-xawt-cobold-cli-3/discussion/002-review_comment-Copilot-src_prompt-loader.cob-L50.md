...
The header comment says `PL-STATUS` = 'N' means "file not found", but the code sets 'N' for any OPEN failure (permission denied, path invalid, etc.). Consider checking `WS-FILE-STATUS` (e.g., "35" for not found) and either returning a distinct status for other errors or updating the comment/behavior so errors aren’t silently treated as "missing".
