...
The loader concatenates lines using a single space and `TRIM(PROMPT-RECORD)`, which removes blank lines/indentation and discards the original line breaks. This prevents multi-line prompts from round-tripping and makes the `CONTEXT-MGR` newline escaping ineffective. Consider preserving lines as-is and inserting an actual newline (X"0A") between records (optionally avoiding an extra newline at the end).
